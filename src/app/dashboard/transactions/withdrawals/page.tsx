  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        url.searchParams.append('type', 'withdrawal');
        ...
        if (!response.ok) {
          throw new Error(`출금 거래 목록을 가져오는데 실패했습니다: ${response.status}`);
        }
        ...
        console.error("출금 거래 목록 로딩 중 오류 발생:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold">출금 거래 관리</h1>
          <p className="text-muted-foreground">출금 거래 내역을 조회하고 관리합니다.</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full mb-6">
        ...
        <TabsContent value="all" className="mt-6">
          <TransactionTable 
            ...
            detailUrlPrefix="/dashboard/transactions/withdrawals"
          />
        </TabsContent>
